...
export default function Sidebar() {
  const { roleLevel } = useAuth(); const isSuperadmin = roleLevel === 100;
  return (
    <aside className="w-64 bg-deep-slate/80 border-r border-white/5 p-4 h-screen sticky top-0">
      <div className="mb-8"><h2 className="text-sm uppercase tracking-wider text-slate-400 font-montserrat font-bold">Navigation</h2></div>
      <nav className="space-y-1">
        ...
        {isSuperadmin && (<><div className="my-4 border-t border-white/5" /><NavItem to="/onboarding" label="Onboarding (Genesis)" className="text-burnt-ochre" /></>)}
      </nav>
    </aside>
  );
}
function NavItem({ to, label, className = '' }) {
  return (<Link to={to} className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors hover:bg-white/5 hover:text-white text-slate-400 ${className}`}>{label}</Link>);
}
